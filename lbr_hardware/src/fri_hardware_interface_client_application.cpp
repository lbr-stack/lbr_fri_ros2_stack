    ...
    namespace FRI {
        ...
        bool FRIHardwareInterfaceClientApplication::receiveAndDecode() {
            if (!_connection.isOpen()) {
                RCLCPP_ERROR(rclcpp::get_logger(CLIENT_APPLICATION_LOGGER), "Client application is not connected.");
                return false;
            }
            ...
            if (buffer_size_ <= 0) {  // TODO: buffer_size_ == 0 -> connection closed (maybe go to IDLE instead of stopping?)
                RCLCPP_ERROR(rclcpp::get_logger(CLIENT_APPLICATION_LOGGER), "Failed while trying to receive monitoring message");
                return false;
            }

            if (!_data->decoder.decode(_data->receiveBuffer, buffer_size_)) {
                RCLCPP_ERROR(rclcpp::get_logger(CLIENT_APPLICATION_LOGGER), "Failed while trying to decode data.");
                return false;
            }
            ...
            {
                RCLCPP_ERROR(
                    rclcpp::get_logger(CLIENT_APPLICATION_LOGGER),
        ...
        bool FRIHardwareInterfaceClientApplication::encodeAndSend() {
            ...
            if (_data->lastSendCounter >= _data->monitoringMsg.connectionInfo.receiveMultiplier) {
                ...
                if (!_data->encoder.encode(_data->sendBuffer, buffer_size_)) {
                    RCLCPP_ERROR(rclcpp::get_logger(CLIENT_APPLICATION_LOGGER),"Failed while trying to encode data.");
                    return false;
                }

                if (!_connection.send(_data->sendBuffer, buffer_size_)) {
                    RCLCPP_ERROR(rclcpp::get_logger(CLIENT_APPLICATION_LOGGER),"Failed while trying to send command message.");
                    return false;
                }
            }
