...
Make sure that the ``update_rate`` in `lbr_controllers.yml <https://github.com/lbr-stack/lbr_fri_ros2_stack/tree/rolling/lbr_hardware_interface/config/lbr_controllers.yml>`_ is greater or equal ``100`` (``FRI send period``).

For using other ``FRI send period``, also change the ``sample_time`` in the `lbr.ros2_control.xacro <https://github.com/lbr-stack/lbr_fri_ros2_stack/blob/rolling/lbr_description/ros2_control/lbr.ros2_control.xacro>`_ (automated in the future).
...
- Frequency: Make sure the ``ros2_control_node`` runs at the same or a higher rate of the ``FRI send period``, change ``update_rate`` in `lbr_controllers.yml <https://github.com/lbr-stack/lbr_fri_ros2_stack/tree/rolling/lbr_hardware_interface/config/lbr_controllers.yml>`_. 
