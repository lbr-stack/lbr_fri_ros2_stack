...
- Parameters, such as ``update_rate``, the configured controllers, are simply set as node parameters, see e.g. `hardware_controllers.yaml <https://github.com/lbr-stack/lbr_fri_ros2_stack/blob/jazzy/lbr_description/ros2_control/hardware_controllers.yaml>`_:octicon:`link-external`.
...
- The ``controller_manager::ControllerManager`` loads the correct plugin from the ``<ros2_control>`` tag in the ``robot_description``: `lbr_system_interface.xacro <https://github.com/lbr-stack/lbr_fri_ros2_stack/blob/jazzy/lbr_description/ros2_control/lbr_system_interface.xacro>`_:octicon:`link-external`
- FRI relevant parameters are forwarded to the ``lbr_ros2_control::SystemInterface`` plugin from `lbr_system_config.yaml <https://github.com/lbr-stack/lbr_fri_ros2_stack/blob/jazzy/lbr_description/ros2_control/lbr_system_config.yaml>`_:octicon:`link-external`
